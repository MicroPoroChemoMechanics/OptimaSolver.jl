...
#   julia --project=docs/ docs/deploy_docs.jl v0.2.1    # specific tag → stable/ + vX.Y.Z/
#
...
cd(pagesdir) do
    run(`git config user.name "OptimaSolver Documenter"`)
    run(`git config user.email "docs@optimasolver.codeberg.page"`)
